...
export default function DashboardPage() {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (res.status === "completed") {
          setAnalysisData(res);
          toast.success("Extraction completed. You can check the results!");
  ...
  if (status === "failed") {
    return (
      <div className="min-h-screen bg-background text-foreground flex flex-col items-center justify-center p-4 relative overflow-hidden">
        ...
        <button onClick={() => router.push("/")} className="mt-8 px-6 py-2 bg-[var(--panel)] border border-[var(--panel-border)] hover:bg-[var(--panel-border)] rounded-xl transition-colors relative z-10">
          Go Back
        </button>
      </div>
    );
  ...
  if (status !== "completed") {
    return (
      <div className="min-h-screen bg-background text-foreground flex flex-col items-center justify-center p-4 relative overflow-hidden">
        ...
        <h1 className="text-2xl font-bold mt-8 mb-2 capitalize relative z-10">
          {status.replace("_", " ")}
        </h1>
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col items-center p-4 md:p-8 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-7xl mb-8 flex items-center justify-between">
         <button onClick={() => router.push("/")} className="flex items-center gap-2 text-foreground/70 hover:text-foreground transition-colors">
